...
def plot_witness_updates_vc_indy(entries, totalVCs, totalRevocations):
    ...
    for level in levels:
        ...
        WitnessUpdatesOfIndy[level] = (totalVCs-1)*(totalRevocations)
    ...
    wuRange = np.linspace(start=50, stop=99900,
                           num=len(x1points))
    ax.set_yscale("log")

    y_major = LogLocator(base=10)
    y_minor = LogLocator(base=10, subs=[10, 20, 30])
    ...
    ax.legend()
    filename ="graphs/witness_updates_indy_vs_ours"+str(totalVCs)+".png"
